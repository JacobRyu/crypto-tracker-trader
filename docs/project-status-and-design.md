...
| Grafana | `deployments/k8s/grafana-datasources.yaml` - Prometheus + Tempo + Loki データソース自動設定 |
| Grafanaダッシュボード | `deployments/k8s/grafana-dashboards.yaml` - Infrastructure Overview ダッシュボード |
| Loki | `deployments/k8s/loki.yaml` - `grafana/loki:3.2.0`、local storage、ログ集約 |
...
アプリ → stdout/stderr → Promtail → Loki:3100（ログ集約）
